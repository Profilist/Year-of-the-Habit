...
export default async function (req, res) {
  ...
  try {
    const completion = await openai.createCompletion({
      ...
      prompt: generateChecklist(req.body.plan),
      max_tokens: 2000,
      temperature: 0.6,
    });
...
function generateChecklist(plan) {
  return `Create a checklist for the steps.
#
Plan: 
...
Identify triggers that lead you to smoke
Seek support from a healthcare professional or smoking cessation program
Quit smoking:
Remove all cigarettes and smoking paraphernalia from your environment
Consider using nicotine replacement therapy (such as gum or patches)
Stay committed and be patient
Stay smoke-free:
Find new ways to cope with stress or triggers
Seek support from friends, family, or a support group
Set small goals and reward yourself for staying smoke-free
